...
通常情况下，我们将声明放在 `.h` 文件中，将定义内容放在 `.cpp` 文件中。可是使用模板时，这种方式就存在问题了。这是因为模板类型不是一种实类型，它必须等到类型绑定后才能确定最终类型，所以在实例化一个模板时，必须要放在能让编译器找到的地方。

既然将 `.h` 文件和 `.cpp` 文件分离的方式存在问题，那么就应该有解决的办法吧？确实是有的，并且有三种方法。

#### 方法一

第一种方法（同时也是最好的方法）是模板的声明和定义内容都在 `.h` 文件中，这种方式是一种最常见的方式，但又破坏了程序的优雅性和可读性；另外这种方式定义的模板编译器会默认当成内联函数，这会引起代码膨胀的隐患。

#### 方法二

在使用模板时不是包含头文件，而是包含 `.cpp` 文件，但是这种方式也有缺陷。
...
使用关键字 “export”，在 `.h` 文件中声明模板，在 `.cpp` 文件中使用关键字 export 来定义。此方法同样有缺陷，不是所有编译器都支持 “export” 关键字。
